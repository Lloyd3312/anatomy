# Anatomy — CI/CD Pipeline on AWS EKS

A production-style CI/CD pipeline for **Anatomy**. This project demonstrates a full GitOps workflow: developers push code to GitHub, Jenkins handles Continuous Integration (build & push Docker images), and ArgoCD handles Continuous Deployment to a Kubernetes cluster on AWS EKS — with Prometheus & Grafana for monitoring.
...
**For new features:**
- Developer creates a new branch from `main`
...
| Resource | Name |
|---|---|
| EC2 Instance (Jenkins/CI server) | `Anatomy-server` |
| EKS Cluster | `anatomy` |
| Docker Hub Repo | `nwachukwuchukwuka123/anatomy` |
| GitHub Repo | `santos-alaska/anatomy` |
| ArgoCD Application | `anatomy` |
| Kubernetes Deployment | `anatomy-app` |
| Kubernetes Service | `anatomy-service` |
| Namespace | `default` |
...
- Attach the role to the `Anatomy-server` EC2 instance (**Actions → Security → Modify IAM role**).

### 6. Create the EKS Cluster
```bash
eksctl create cluster --name anatomy --region us-east-1 --node-type t2.medium --zones us-east-1a,us-east-1b
```
...
- Create the GitHub repo `anatomy`
- Add the application source, `Dockerfile`, `k8s/` manifests, and `Jenkinsfile`
- Push to `main`

### 10. Configure kubectl Access for the Jenkins User

> **Note:** Instead of copying root's kubeconfig over to the `jenkins` user, AWS/Kubernetes access was configured **directly as the `jenkins` user** — avoiding the need to `mkdir`/`cp`/`chown` root's kubeconfig into `/home/jenkins/.kube/`.
...
# Generate a kubeconfig for the jenkins user, pointing at the anatomy cluster
aws eks update-kubeconfig --region us-east-1 --name anatomy
...
- Display name: `Anatomy-App`
- Description: *Production-grade deployment pipeline for Anatomy*
...
- Source:
  - Repository URL: `https://github.com/santos-alaska/anatomy.git`
  - Revision: `main`
  - Path: `k8s`
- Destination:
...
```
Copy the `EXTERNAL-IP` of `anatomy-service` and open it in a browser to see the live app — this is **v1** of the project.
...
- Open a Pull Request on GitHub (`new-branch` → `main`)
- Reviewer reviews and **merges** the PR
- click **Build Now** manually in jenkins
...
```bash
eksctl delete cluster --name anatomy --region us-east-1
```
